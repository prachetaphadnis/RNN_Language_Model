...
class RNN(object):
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            net_in=0
            net_out=0
            w=make_onehot(x[t],self.vocab_size)
            w_t = np.dot(self.V,w)
            if t==0:
                net_in=w_t+np.dot(self.U,s[0])
            else:
                net_in=w_t+np.dot(self.U,s[t-1])
            s[t]=sigmoid(net_in)
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        no return values
        '''
        #accumulate updates for W 
        for t in reversed(range(len(x))):

            g_dsh = 1
            d_onehot=make_onehot(d[t],self.vocab_size)
            delta_out = np.dot(d_onehot - y[t], g_dsh)
            #print(np.outer(delta_out, s[t]))
            #print(delta_out.shape, s[t].shape)
            self.deltaW += np.outer(delta_out, s[t])            
            # accumulate updates for V
            
            f_dsh = np.multiply(s[t], np.ones(len(s[t])) - s[t])
         
            #print(np.dot(delta_out, f_dsh))
            delta_in = np.dot(np.transpose(self.W),delta_out)
         
            delta_in =np.multiply(delta_in, f_dsh)
    ...
    def acc_deltas_np(self, x, d, y, s):
        ...
        delta_out = np.dot(d_onehot - y[-1], g_dsh)
            #print(np.outer(delta_out, s[t]))
            #print(delta_out.shape, s[t].shape)
        self.deltaW += np.outer(delta_out, s[-2])
        f_dsh = np.multiply(s[-2], np.ones(len(s[-2])) - s[-2])
        
        delta_in = np.dot(np.transpose(self.W),delta_out)
         
        delta_in =np.multiply(delta_in, f_dsh)

        self.deltaV += np.outer(delta_in, make_onehot(x[-1],self.vocab_size))
         
        if len(s)>2:
            self.deltaU += np.outer(delta_in, s[-3])
        else:
            self.deltaU += np.outer(delta_in, s[-2])
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
        ...
        for t in reversed(range(len(x))):
            ...
            for tau in range(1,min(steps+1,len(x)+1)):
                
                f_dsh_tau = np.multiply(s[t-tau], np.ones(len(s[t-tau])) - s[t-tau])
                delta_in = np.multiply(np.dot(np.transpose(self.U),delta_in),f_dsh_tau)
    ...
    def compute_loss(self, x, d):
        ...
        for t in range(len(x)):
            d_t = make_onehot(d[t], self.vocab_size)
            j_sum=0
            for j in range(self.vocab_size):
                j_sum +=  d_t[j] * np.log(y[t,j])
            J[t]=-j_sum
    ...
    def compute_loss_np(self, x, d):
        ...
        y, s = self.predict(x)
        J=0
        d_t = make_onehot(d[0], self.vocab_size)
        j_sum=0
        for j in range(self.vocab_size):
            j_sum +=  d_t[j] * np.log(y[-1,j])
        J=-j_sum

        loss=np.sum(J)
        #f=open("predicts_loss.txt",'a')
    ...
    def compute_acc_np(self, x, d):
        ...
        x        list of words, as indices, e.g.: # --- your code here --- #[0, 4, 2]
    ...
    def train(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
        ...
        anneal            positive integer. if > 0, lowers the learning rate in a harmonically after each epoch.
                        ...
                        anneal=0 willoss_functionl not change the learning rate over time.
                        default 5
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        loss=rnn.train(X_train,D_train,X_dev,D_dev,epochs=10,learning_rate=lr, back_steps=lookback, log=True)
        
        run_loss = -1
        adjusted_loss = -1
        run_loss=rnn.compute_mean_loss(X_dev,D_dev)
        fracloss=fraq_loss(vocab, word_to_num, vocab_size)
        adjusted_loss=adjust_loss(run_loss, fracloss, q)
    ...
    if mode == "train-np":
        ...
        run_loss=rnn.train_np(X_train,D_train,X_dev,D_dev,epochs=10,learning_rate=lr, back_steps=lookback, log=True)
    ...
    if mode == "test-lm":
        ...
        S_np_test = docs_to_indices(sents, word_to_num, 1, 1)
        X_np_test, D_np_test = seqs_to_lmXY(S_np_test)
        print(D_np_test.shape)

        mean_loss_test=r.compute_mean_loss(X_np_test,D_np_test)
        #print('Number prediction accuracy on test set:', np_acc_test)
        print('Mean loss on test set:', mean_loss_test)
        q = vocab.freq[vocab_size] / sum(vocab.freq[vocab_size:])
        fracloss=fraq_loss(vocab, word_to_num, vocab_size)
        adjusted_loss=adjust_loss(mean_loss_test, fracloss, q)
        print("Unadjusted: %.03f" % np.exp(mean_loss_test))
        print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))
    
    if mode== "train-np-grammar":
        ...
        cnt_ungram,total_sents,docs = load_grammar_dataset(data_folder+'/wiki-train.txt')
    
        vocab = pd.read_table(data_folder+"/vocab.wiki.txt", header=None, sep="\s+", index_col=0, names=['count', 'freq'], )
        total_words=np.sum(vocab['count'])
        freq_ungram=float(cnt_ungram)/(total_words+total_sents)
        freq_gram=float(total_sents-cnt_ungram)/(total_words+total_sents)

        for i in range(len(vocab)):
            vocab.ix[i,'freq']=float(vocab.ix[i,'count'])/(total_words+total_sents)

    
        d = {'count': [cnt_ungram,total_sents-cnt_ungram], 'freq': [freq_ungram, freq_gram]}
        df = pd.DataFrame(data=d, index=['cat_ungrammatical','cat_grammatical'])
    
        new_vocab=pd.concat([vocab,df])
        vocab=new_vocab
        keep_vocab=(vocab.index[:vocab_size]).append(vocab.index[-2:])

        num_to_word = dict(enumerate(keep_vocab))
        word_to_num = invert_dict(num_to_word)
        
        # calculate loss vocabulary words due to vocab_size
        fraction_lost = fraq_loss(vocab, word_to_num, vocab_size+2)
        print("Retained %d words from %d (%.02f%% of all tokens)\n" % (vocab_size+2, len(vocab), 100*(1-fraction_lost)))
        
        _,_,docs = load_grammar_dataset(data_folder+'/wiki-train.txt')
        S_train = docs_to_indices(docs, word_to_num, 0, 0)
        X_train, D_train = seqs_to_npXY(S_train)

        # Load the dev set (for tuning hyperparameters)
        _,_,docs = load_grammar_dataset(data_folder+'/wiki-dev.txt')
        ...
        run_loss = -1
        rnn=RNN(vocab_size+2,hdim,vocab_size+2)
        ...
        fracloss=fraq_loss(vocab, word_to_num, vocab_size+2)
        q = vocab.freq[vocab_size+2] / sum(vocab.freq[vocab_size+2:])
        adjusted_loss=adjust_loss(run_loss, fracloss, q)
        print("Unadjusted: %.03f" % np.exp(run_loss))
        print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))
        np.save('rnn.U_gram',rnn.U)
        np.save('rnn.V_gram',rnn.V)
        np.save('rnn.W_gram',rnn.W)
    
        
        acc = 0.
        
        
    if mode == "train-np-noun":
        '''
        starter code for parameter estimation.
        change this to different values, or use it to get you started with your own testing class
        '''
        ...
        # load training data
        sents = load_noun_dataset(data_folder + '/wiki-train.txt')
        S_train = docs_to_indices(sents, word_to_num, 0, 0)
        X_train, D_train = seqs_to_npXY(S_train)
        ...
        # load development data
        sents = load_noun_dataset(data_folder + '/wiki-dev.txt')
        S_dev = docs_to_indices(sents, word_to_num, 0, 0)
        X_dev, D_dev = seqs_to_npXY(S_dev)
        ...
        print("Unadjusted: %.03f" % np.exp(run_loss))
        print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))
        np.save('rnn.U_noun',rnn.U)
        np.save('rnn.V_noun',rnn.V)
        np.save('rnn.W_noun',rnn.W)
        
        acc = 0.

    if mode == "test-np-noun":
        
        data_folder = sys.argv[2]
        rnn_folder = sys.argv[3]

        # get saved RNN matrices and setup RNN
        U,V,W = np.load(rnn_folder + "/rnn.U_noun.npy"), np.load(rnn_folder + "/rnn.V_noun.npy"), np.load(rnn_folder + "/rnn.W_noun.npy")
        vocab_size = len(V[0])
        hdim = len(U[0])
        ...
        # load test data
        sents = load_noun_dataset(data_folder + '/wiki-test.txt')
        S_np_test = docs_to_indices(sents, word_to_num, 0, 0)
        X_np_test, D_np_test = seqs_to_npXY(S_np_test)

        mean_loss_test = sum([r.compute_loss_np(X_np_test[i], D_np_test[i]) for i in range(len(X_np_test))]) / len(D_np_test)

        print('Mean loss on test set:', mean_loss_test)
        
        acc=sum([r.compute_acc_np(X_np_test[i], D_np_test[i]) for i in range(len(X_np_test))]) / len(X_np_test)
    ...
    if mode == "test-np-grammar":
        
        data_folder = sys.argv[2]
        rnn_folder = sys.argv[3]

        # get saved RNN matrices and setup RNN
        U,V,W = np.load(rnn_folder + "/rnn.U_gram.npy"), np.load(rnn_folder + "/rnn.V_gram.npy"), np.load(rnn_folder + "/rnn.W_gram.npy")
        vocab_size = len(V[0])
        hdim = len(U[0])
        ...
        new_vocab=pd.concat([vocab,df])
        vocab=new_vocab
        keep_vocab=(vocab.index[:vocab_size-2]).append(vocab.index[-2:])
   
        num_to_word = dict(enumerate(keep_vocab))
        word_to_num = invert_dict(num_to_word)

        # load test data
        _,_,sents = load_grammar_dataset(data_folder + '/wiki-test.txt')
        S_np_test = docs_to_indices(sents, word_to_num, 0, 0)
        X_np_test, D_np_test = seqs_to_npXY(S_np_test)
        print(D_np_test.shape)
